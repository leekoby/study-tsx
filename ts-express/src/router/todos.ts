...
todoRouter.post(
  ...
  async (
    ...
    try {
      // if (!req.user) return res.status(403).send('로그인후에 접근해주세요!'); //여기수정함
...
todoRouter.get('/', async (req: Request, res: Response<Todo[] | string>, next: NextFunction) => {
  try {
    // if (!req.user) return res.status(403).send('로그인후에 접근해주세요!');//여기수정함

    // const targetTodos = todos.filter((todo) => todo.userId === req.user?.id); 여기수정함
    const targetTodos = todos.filter((todo) => todo.userId === 1);
...
todoRouter.delete(
  '/:id',
  async (req: Request<{ id: string }>, res: Response<Todo>, next: NextFunction) => {
    try {
      const id = +req.params.id;
